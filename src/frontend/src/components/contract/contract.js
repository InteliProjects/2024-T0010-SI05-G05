...
class Contract {
    ...
    async addUser(adress, userName){
        try {
            ...
            let response = this.contract.methods.addUser(adress, userName)
            .send({ from: user  }) // substitua '0xYourAccountAddress' pelo endereço da sua conta
            return response
        } catch(error) {
            console.log("error")
            return false
    ...
    async deleteUser(adress){
        try {
            ...
            const user = accounts[0];
            console.log("adress", adress);
            let response = this.contract.methods.deleteUser(adress)
            .send({ from: user  }) // substitua '0xYourAccountAddress' pelo endereço da sua conta
            return response
        } catch(error) {
